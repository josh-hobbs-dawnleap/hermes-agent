...
| `TELEGRAM_AMBIENT_CHATS` | Comma-separated Telegram chat IDs or `chat_id:thread_id` topic entries where ambient classification may proactively wake the agent. Equivalent to `telegram.ambient_chats`. |
| `TELEGRAM_AUTO_TRUST_GROUPS_FROM_AUTHORIZED_SENDERS` | When `true`, Telegram group observation/trigger attribution may trust the current group turn if the current sender is already authorized through the normal Telegram user allowlist or pairing path. This is a no-known-group-ID bootstrap; unknown senders remain silent and the group is not blanket-authorized. Equivalent to `telegram.auto_trust_groups_from_authorized_senders`. |
| `TELEGRAM_AUTO_AMBIENT_CHATS_FROM_AUTHORIZED_SENDERS` | When `true`, the ambient classifier may run for authorized-sender group turns even before the chat is explicitly listed in `TELEGRAM_AMBIENT_CHATS`. Requires ambient mode and the auto-trust policy; replace with explicit ambient chat IDs after discovery. Equivalent to `telegram.auto_ambient_chats_from_authorized_senders`. |
